...
const DocumentParser: React.FC = () => {
  ...
  const [question, setQuestion] = useState<string>('Extract all text and structure from this document.');
  ...
  const handleSubmit = async () => {
    if (!image || !question) {
      alert('Please upload a document image and enter a request.');
      return;
    }
    setLoading(true);
    setResult('');
  ...
  return (
    <FeatureContainer
      ...
      description="Upload an image of a document (e.g., a page from a PDF, a screenshot of a webpage) to extract text and structure."
    >
      <div className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-2">Upload Document Image</label>
          <input
            type="file"
            accept="image/*"
          ...
          />
          {image && <img src={image.preview} alt="Preview" className="mt-4 rounded-lg max-h-96 w-full object-contain border border-slate-700" />}
        </div>

        <div>
          <label htmlFor="question" className="block text-sm font-medium text-slate-300 mb-2">Instructions</label>
          <textarea
            id="question"
            value={question}
            ...
            placeholder="e.g., Extract all text and structure from this document."
          />
        </div>
